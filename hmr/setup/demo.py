...
def visualize(img_path, img, proc_param, joints, verts, cam):
    ...
    #plt.axis('off')
    #plt.draw()
    rend_img = cv2.cvtColor(skel_img, cv2.COLOR_BGR2RGB)
    rend_img_overlay = cv2.cvtColor(rend_img_overlay, cv2.COLOR_BGR2RGB)
    cv2.imwrite("hmr/output/images/mesh/"+os.path.splitext(os.path.basename(img_path))[0]+".png",rend_img)
    cv2.imwrite("hmr/output/images/overlay/"+os.path.splitext(os.path.basename(img_path))[0]+".png",rend_img_overlay)
    # import ipdb
    # ipdb.set_trace()
